...
* `radius`: add the trust-region constraint ‖x‖ ≤ `radius` if `radius > 0`. Useful to compute a step in a trust-region method for optimization.
  ...
  - If nonpositive curvature is detected along the current search direction `w`, we take the step to the trust-region boundary.
    ...
    `stats.npcCount` is set to 1, or to 2 at iteration k > 1 if `w` is a direction of nonpositive curvature as well; the step follows `r` either way.
  ...
  function minres_qlp!(workspace :: MinresQlpWorkspace{T,FC,S}, $(def_args_minres_qlp...); $(def_kwargs_minres_qlp...)) where {T <: AbstractFloat, FC <: FloatOrComplex{T}, S <: AbstractVector{FC}}
    ...
    while !(solved || tired || inconsistent || ill_cond_mach || breakdown || on_boundary || user_requested_exit || overtimed)
      ...
      if npc
        ...
        # which is a direction of nonpositive curvature, whether or not wₖ is one as well.
        # Follow it from xₖ₋₁ to the boundary of the trust region.
        # The curvature along wₖ is only used to report the number of directions detected.
        # wₖ₋₁ is no longer needed and is never aliased to vₖ₊₁, so it serves as scratch.
        w_npc = false
        if iter > 1
          kmul!(wₖ₋₁, A, wₖ)
          wNorm² = kdotr(n, wₖ, wₖ)
          w_curvature = kdotr(n, wₖ, wₖ₋₁) + λ * wNorm²
      ...
      # Advance the preconditioned residual held in npc_dir from rₖ₋₁ to rₖ, for the
      # curvature check of the next iteration. Both exits above return rₖ₋₁, so this
      # update is only reached when the iterations continue.
      # It must happen before vₖ₊₁ is consumed just below.
      if linesearch || (radius > 0)
