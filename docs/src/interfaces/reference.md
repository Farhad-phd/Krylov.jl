...
- `radius`, a trust-region constraint `‖x‖ ≤ radius` for CG, CR, CGLS, CRLS, LSQR, LSMR, MINRES and MINRES-QLP. `0.0` (default) means unconstrained; with `radius > 0` the step stops on the trust-region boundary. Useful inside optimization solvers.
